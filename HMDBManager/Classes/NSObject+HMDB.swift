...
public extension NSObject {
    ...
    public class var tableName:String{
    ...
    public func getMaxDefaultPK()->Int{
    ...
    //MARK:增删改查
    @objc public  func dbAdd(completion: ((Bool)->Void)?){
        self.dbSave(insert: true , completion: completion)
    }
    @objc public  func dbUpdate(completion: ((Bool)->Void)?){
        self.dbSave(insert: false , completion: completion)
    }
    
    @objc public  func dbSave(completion: ((Bool)->Void)?){
        self.dbSave(insert: nil , completion: completion)
    }
    
    public func dbSave(insert:Bool? ,completion:((Bool)->Void)?){
...
//                disableHMDBLog ? () : debugPrint("HMDB save sql \(result):\(sql) values:\(dbvalues)")
    ...
    @objc public  func dbDelete(completion:((Bool)->Void)?){
        self.handleInDBqueue().inDatabase({ (db ) in
            let tableName:String = "\(self.classForCoder)"
    ...
    @objc public  class func dbQuery(whereStr:String?,orderFields:String?,offset:Int,limit:Int,args:[Any],completion:@escaping (([Any],Error?)->Void)){
